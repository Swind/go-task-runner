...
type JobManager struct {
	...
	controlRunner   taskrunner.TaskRunner // Layer 1: Fast control operations
	ioRunner        taskrunner.TaskRunner // Layer 2: Sequential IO operations
	executionRunner taskrunner.TaskRunner // Layer 3: User job execution
...
func (m *JobManager) doRecoveryIO(ctx context.Context) error {
	...
	for _, job := range jobs {
		...
		m.controlRunner.PostTask(func(_ context.Context) {
			...
			// Schedule execution
			traits := taskrunner.TaskTraits{Priority: taskrunner.TaskPriority(jobCopy.Priority)}
