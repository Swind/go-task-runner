...
// TestParallelTaskRunner_GC_TaskWithStructMethod verifies struct method GC
...
func TestParallelTaskRunner_GC_TaskWithStructMethod(t *testing.T) {
...
// TestParallelTaskRunner_GC_ClosureCapturedObjects verifies closure-captured object GC
...
func TestParallelTaskRunner_GC_ClosureCapturedObjects(t *testing.T) {
...
// TestParallelTaskRunner_GC_RepeatingTaskStopped verifies repeating task object GC
...
func TestParallelTaskRunner_GC_RepeatingTaskStopped(t *testing.T) {
...
// TestParallelTaskRunner_GC_ShutdownClearsQueue verifies pending task GC on shutdown
...
func TestParallelTaskRunner_GC_ShutdownClearsQueue(t *testing.T) {
	// Arrange - Create pool, runner, and block with long tasks
	pool := taskrunner.NewGoroutineThreadPool("test-pool", 4)
	pool.Start(context.Background())
	defer pool.Stop()

	// Use maxConcurrency=1 to ensure tasks queue up
...
// TestParallelTaskRunner_GC_TaskAndReplyPattern verifies PostTaskAndReply object GC
// Given: an object captured in both task and reply closures
// When: task and reply complete
// Then: the object is garbage collected and finalizer is called
func TestParallelTaskRunner_GC_TaskAndReplyPattern(t *testing.T) {
	// Arrange - Create pool, runners, and object with finalizer
	pool := taskrunner.NewGoroutineThreadPool("test-pool", 4)
	pool.Start(context.Background())
	defer pool.Stop()

	bgRunner := core.NewParallelTaskRunner(pool, 2)
	uiRunner := core.NewParallelTaskRunner(pool, 2)
...
// TestParallelTaskRunner_GC_DelayedTask verifies delayed task object GC
// Given: an object captured by a delayed task
// When: the delayed task executes
// Then: the object is garbage collected and finalizer is called
func TestParallelTaskRunner_GC_DelayedTask(t *testing.T) {
...
// TestParallelTaskRunner_GC_MemoryGrowth verifies no unbounded memory growth
// Given: a ParallelTaskRunner executing 1000 tasks with 100KB allocations each
// When: all tasks complete and GC runs
// Then: memory growth is less than 10MB (no memory leak)
func TestParallelTaskRunner_GC_MemoryGrowth(t *testing.T) {
	if testing.Short() {
...
// TestParallelTaskRunner_GC_RunnerItself verifies runner can be GC'd
// Given: a ParallelTaskRunner that has executed tasks and been shutdown
// When: all references are dropped
// Then: the runner is garbage collected and finalizer is called
func TestParallelTaskRunner_GC_RunnerItself(t *testing.T) {
	// Arrange - Create pool
	pool := taskrunner.NewGoroutineThreadPool("test-pool", 4)
	pool.Start(context.Background())
	defer pool.Stop()
	...
	func() {
		...
		runtime.SetFinalizer(runner, func(r *core.ParallelTaskRunner) {
			finalizerCalled.Store(true)
			wg.Done()
		})
...
// TestParallelTaskRunner_GC_TaskAndReplyWithResult verifies result-passing pattern GC
// Given: an object captured in PostTaskAndReplyWithResult
// When: task and reply complete
// Then: the object is garbage collected and finalizer is called
func TestParallelTaskRunner_GC_TaskAndReplyWithResult(t *testing.T) {
	// Arrange - Create pool, runners, and object with finalizer
	pool := taskrunner.NewGoroutineThreadPool("test-pool", 4)
	pool.Start(context.Background())
	defer pool.Stop()
...
// TestParallelTaskRunner_GC_FlushAsyncBarrier verifies FlushAsync callback GC
// Given: an object captured by a FlushAsync callback
// When: the callback executes
// Then: the object is garbage collected and finalizer is called
func TestParallelTaskRunner_GC_FlushAsyncBarrier(t *testing.T) {
	// Arrange - Create pool, runner, and object with finalizer
	pool := taskrunner.NewGoroutineThreadPool("test-pool", 4)
	pool.Start(context.Background())
	defer pool.Stop()
	...
	func() {
		...
		// Post a barrier that captures the object
		done := make(chan struct{})
		...
		case <-done:
		case <-time.After(2 * time.Second):
			t.Fatal("timeout waiting for FlushAsync")
		}
	}()
...
// TestParallelTaskRunner_GC_ConcurrentExecution verifies GC with parallel execution
// Given: multiple tasks running concurrently, each capturing an object
// When: tasks complete
// Then: all objects are garbage collected
func TestParallelTaskRunner_GC_ConcurrentExecution(t *testing.T) {
	// Arrange - Create pool and runner with higher concurrency
	...
	case <-done:
		// Assert - Verify all objects collected
		collected := finalizerCount.Load()
		if collected != numTasks {
			t.Errorf("concurrent tasks GC'd: got = %d, want = %d", collected, numTasks)
		}
	case <-time.After(3 * time.Second):
		collected := finalizerCount.Load()
		t.Errorf("timeout: only %d/%d concurrent task objects were GC'd", collected, numTasks)
	}
}
